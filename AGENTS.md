...
- 코드 작업은 [workflow.md](./.claude/rules/workflow.md)를 따른다.
- 가독성이 최우선이다. 성능 최적화보다 읽기 쉬운 코드를 우선한다.
- 세부 규칙은 rule 각 파일을 본다.
- 문서는 A4 2장 미만으로 작성한다.
...
문서 작성과 검토에 `akbun-*` plugin을 사용한다. Claude Code인경우 marketplace 등록과 plugin 설치한다. codex는 로컬에 이미 설치되어 있다고 가정한다.

```bash
# claude code plugin 설치 방법
