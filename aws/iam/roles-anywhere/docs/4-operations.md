...
- 실습은 trust anchor ARN·계정·CN을 제한해요. 운영 속성은 인증서 발급 절차가 보장해야 해요.
- 클라이언트가 CSR의 CN을 자유롭게 지정하고 CA가 그대로 서명하면 속성 기반 제한의 의미가 약해져요.
- 같은 CN의 여러 인증서는 같은 Role 조건에 맞아요. 한 CN을 공유하면 개별 workload 구분이 어려워져요.
...
- 이 샘플의 Python 경로 검사는 일반 파일용이에요. HSM URI나 OS selector를 사용할 때는 helper 명령 생성 부분을 맞춰야 해요. [공식 helper의 키 저장소 지원](https://docs.aws.amazon.com/rolesanywhere/latest/userguide/credential-helper.html)

## 인증서 교체

1. 기존 인증서가 유효할 때 새 개인 키·인증서를 발급해요.
2. 새 체인과 workload 식별 속성을 검증해요.
3. 인증서와 키를 하나의 배포 단위로 전환해요.
...
- 새 프로세스로 성공하는 것과 이미 실행 중인 SDK의 다음 갱신이 성공하는 것을 모두 확인해요.
- CA 교체에는 새로운 trust anchor·Role trust 변경·기존 체인의 종료 순서까지 필요해요.
- AWS Private CA를 trust anchor로 사용할 때 CA 유효기간 변경은 UpdateTrustAnchor 반영도 확인해요. [Trust anchor 유효성](https://docs.aws.amazon.com/rolesanywhere/latest/userguide/trust-model.html)
...
- CA 발급 DB와 serial 관리가 유실되면 정상적인 폐기 목록 생성이 어려워져요.
- 이 실습의 CRL 유효기간은 2일이에요. 운영 주기는 장애 복구에 필요한 여유와 함께 정해요.
...
- 정책·CRL 변경의 전파 지연을 포함해 차단 여부를 재확인해요. [AWS의 Role 세션 권한 회수](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use_revoke-sessions.html)
...
- 갱신 시 Roles Anywhere 접속 실패, 인증서 기간 만료, 파일 권한 변경, 잘못된 profile 설정이 모두 장애 원인이 될 수 있어요.
...
Leaf 인증서가 24시간 이내에 만료되는지 검사해요. 임박하면 종료 코드 1이에요.
