...
- 인증서로 받은 임시 키를 boto3가 사용해 서울 AgentCore Memory에 이벤트를 저장·조회·삭제해요.
...
처음 자격증명을 받을 때와 Memory를 호출할 때의 서명 키가 달라요.
...
- helper의 stdout은 자격증명 전달 채널이에요. 애플리케이션 로그로 출력하지 않아요.
- 클라이언트의 필수 데이터 통신은 Roles Anywhere와 Memory 두 endpoint예요.
...
| Role의 서비스 권한 | Memory의 허용하지 않은 API나 다른 Memory 접근 |
...
- 최종 Role 권한은 실습 Memory 하나의 CreateEvent·GetEvent·DeleteEvent예요. [인증 조건과 Role trust](https://docs.aws.amazon.com/rolesanywhere/latest/userguide/trust-model.html)

## SDK가 갱신하는 방식

- `credential-process` 명령은 한 번 실행되고 종료돼요.
- SDK는 응답의 `Expiration`을 읽고 필요할 때 그 명령을 다시 실행해요.
- 같은 boto3 세션과 client를 유지해야 갱신 가능한 provider가 계속 사용돼요.
- 임시 키를 따로 복사해 `boto3.Session(aws_access_key_id=...)`를 만들면 원래 provider와 연결이 끊겨요.
- 실습은 명시적 SDK profile을 사용해 환경변수의 기존 AWS 키가 먼저 선택되는 상황을 방지해요.
...
- 그 결과가 IAM Role의 MaxSessionDuration을 넘으면 오류예요. Role 한도까지 자동으로 줄여 주는 방식은 아니에요.
...
- NLB 주소로 endpoint만 바꾸면 AWS 서버 인증서와 URL의 이름 문제가 남아요.
