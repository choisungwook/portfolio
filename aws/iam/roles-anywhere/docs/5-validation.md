...
- 로컬 CRL 성공이 AWS의 CRL 배포·정책 전파 검증을 대신하지 않아요.
...
- 확인한 1.8.5 파일 키 경로에서는 `--certificate`도 필요했어요.
- 종료 코드 0은 로컬 서명 성공이에요. AWS의 trust·권한 검증 결과는 아니에요.

| 증상 | 먼저 확인할 것 |
| --- | --- |
| python -m scripts.create_certificates가 기존 디렉터리 오류 | 발급 DB 덮어쓰기 방지 동작. 기존 실습 정리 여부 확인 |
...
| 인증서 교체 후 계속 이전 신원 | 기존 세션 캐시와 다음 갱신 시 실제 helper 파일 선택 |
...
- 실습 밖의 설정이 남았다면 [환경 준비](2-setup.md#up)의 초기화부터 확인해요.
