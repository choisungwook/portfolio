...
- 정상 호출 결과와 실패 원인을 구분해 기록해요. timeout은 인증 거부가 재현된 결과가 아니에요.

## 실험 1. 정상 인증과 Memory 접근

인증서로 자격증명을 받고 Memory 이벤트를 한 번 저장·조회·삭제해요.
...
성공 시 다음 형식의 출력이 나와요. 실제 AWS 성공 로그를 미리 기록한 것은 아니에요.
...
- Memory 쓰기만으로 끝내지 않고 읽은 내용의 일치와 삭제까지 확인해요.
...
- 이 명령의 환경변수는 한 번의 실행에만 적용돼요.
- AccessDenied가 아닌 TLS·DNS·timeout 오류는 별도 문제로 조사해요.

## 실험 3. 인증서와 개인 키 교체

같은 CN에 새 개인 키와 새 serial의 v2 인증서를 발급해요.
...
- v1은 아직 유효해요. 새 버전 성공을 확인한 뒤 v1 폐기로 넘어가요.
- 두 경로 중 하나만 교체하면 인증서와 개인 키가 맞지 않아 실패해요.
...
- 코드: [CRL 등록](../scripts/import_crl.py). 이미 등록했다면 `python -m scripts.update_crl` 사용.
...
- 첫 호출: v1의 새 CreateSession 거부. 두 번째 호출: 환경변수로 선택한 v2의 PASS.
- CA 공개 인증서를 로컬에서 검증하는 것과 AWS의 CRL 반영은 별도예요.
- 전파 지연을 고려해 제한된 횟수로 재시도해요. 거부 원인과 v2 성공을 함께 확인해요.
- v1으로 이미 발급받은 세션까지 폐기됐다는 의미는 아니에요. [기존 세션 권한 회수](4-operations.md#사고-대응과-세션-권한-회수)를 별도로 봐요.

## 실험 5. 한 프로세스에서 자격증명 갱신
...
- 같은 관찰 시간의 CloudTrail에 새 CreateSession이 생겼는지도 확인해요.
...
- 기본 실습 검증 범위는 [검증 결과](5-validation.md)에 기록해요.
