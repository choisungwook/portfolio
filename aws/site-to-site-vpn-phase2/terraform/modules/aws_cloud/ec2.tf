resource "aws_security_group" "test_vm_sg" {
  name        = "${var.project_name}-cloud-test-vm-sg"
  description = "Security group for the test VM in the cloud VPC"
...
resource "aws_instance" "test_vm" {
  ...
  tags = {
    Name    = "${var.project_name}-cloud-nginx"
    Project = var.project_name
  }
}
