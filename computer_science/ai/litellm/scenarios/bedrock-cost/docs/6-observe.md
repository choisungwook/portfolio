...
`increase()`나 `rate()`로 counter의 증가분을 뽑아 비교하려는 시도는 권하지 않습니다. 이 실험은 요청이 몇 초 안에 몰리는 형태라, Prometheus가 구간 양 끝을 보고 추정하는 값이 실제보다 크게 나옵니다. 제가 20분 구간으로 재 봤더니 53,156이어야 할 값이 44,058로 나왔습니다.
