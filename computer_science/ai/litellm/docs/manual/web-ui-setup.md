...
빈 gateway를 띄운 뒤 관리 콘솔(/ui)에서 모델과 virtual key, team을 손수 만든다. UI에서 만든 것이 사라지지 않으려면 [manual/config.yaml](../../scenarios/manual/config.yaml)에 `store_model_in_db: true`가 있어야 한다. 이 값이 UI로 추가한 모델을 Postgres에 저장한다.
...
```bash
curl -s http://localhost:4010/v1/models -H "Authorization: Bearer $LITELLM_MASTER_KEY"
```
...
라우팅·fallback·guardrail은 콘솔 폼이 아니라 config.yaml에 선언한다. manual/ 트랙에서도 이 부분이 필요하면 config.yaml에 적고 재기동한다. 선언 예시는 [set-model/config.yaml](../../scenarios/set-model/config.yaml)에 있다.
