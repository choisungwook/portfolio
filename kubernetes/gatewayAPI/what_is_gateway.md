...
- Kubernetes ingress 문서 링크: https://kubernetes.io/docs/concepts/services-networking/ingress/

![frozen_ingress](./imgs/frozen_ingress.png)

## ingress 개발을 중단하고 Gateway API를 만든 이유는?

Gateway API 문서에서 Gateway API를 만들게 된 3가지 이유를 설명합니다.

- Gateway API 문서 링크: https://gateway-api.sigs.k8s.io/guides/getting-started/migrating-from-ingress/#reasons-to-switch-to-gateway-api

1. ingress는 HTTP, HTTPS만 처리할 수 있습니다.
2. ingress 설정을 annotations로만 하기에는 확장성 제약이 있습니다.
3. ingress 설정을 여러 팀과 협력할 때 어렵습니다.

![why_switch_gatewayapi](./imgs/why_switch_gatewayapi.png)

## 설정 방법

Gateway API의 설정은 최소 3가지입니다. 최소 3가지가 필요한 이유는 요청을 받고 처리하는 작업에 필수적인 요소이기 때문입니다. 요청을 처리하는 것 이외에 TLS처럼 보안 옵션, 요청을 제한하는 등 트래픽 관리(Traffic management) 등 여러 옵션이 있습니다.

1. 누가 또는 어떤 pod가 Gateway API 작업을 수행할 것인가 (gateway controller)?
2. 어떤 요청을 받을 것인가?
3. 요청을 받으면 어떤 pod로 라우팅할 것인가?

![gatewayapi_arch](./imgs/gatewayapi_arch.png)
...
- Gateway API controller 현황: https://gateway-api.sigs.k8s.io/implementations/#gateway-controller-implementation-status

![gateway_controller](./imgs/gateway_controller.png)
...
![gatewayclass](./imgs/gatewayclass.png)
...
![envoy_proxy](./imgs/envoy_proxy.png)
