...
Break compound terms into parts, explain each, then combine. akbun's signature pedagogical device — but **use sparingly**.

This technique is powerful when a term is genuinely composed of meaningful sub-parts and breaking it down helps the reader build understanding step by step. However, not every concept benefits from decomposition. If the term is already well-known or self-explanatory, decomposing it adds unnecessary length without aiding comprehension.

**When to use:**

- The term is a compound of 2+ distinct concepts that each need explanation (e.g., "Site to Site VPN" = Site + Site to Site + VPN)
- The reader likely doesn't know one or more of the sub-parts
- Breaking it down genuinely builds understanding layer by layer

**When NOT to use:**

- The term is already widely understood (e.g., "API Gateway", "Load Balancer")
- The sub-parts don't individually contribute to understanding the whole
- A simple one-line definition is sufficient
...
- **"정리하면" pattern**: "정리하면 ~입니다"는 설명을 마무리할 때 사용할 수 있지만, 매 섹션마다 반복하면 글이 단조로워진다. 글 전체에서 1~2회 정도, 핵심 개념을 최종 정리할 때만 사용한다. 나머지 마무리에는 "따라서", "즉", "결국" 등 다른 접속사를 활용한다.
