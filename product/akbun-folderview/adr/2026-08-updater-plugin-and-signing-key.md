...
One state worth knowing. If the signing secrets are missing from the job, no signature file is produced and the release action skips `latest.json` without failing, which is a green release nobody can update from. See [Release from the package.json version](./2026-08-release-workflow.md).
