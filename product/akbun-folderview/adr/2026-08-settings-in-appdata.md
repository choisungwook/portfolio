...
Store `settings.json` and `library.json` in the application config directory, which on Windows resolves to `%APPDATA%\io.akbun.folderview`. Write both through a temp file and a rename. Show the folder in Settings with a button that opens it.
...
`%APPDATA%` is the answer for per-user state that should follow the user, and `app_config_dir()` is how the framework names it without hard coding a path. It also means the same code finds the right place on macOS and Linux, which keeps `npm start` working while developing on another platform.

The temp file and rename matter more here than in most apps. The settings file is small and easy to recreate; the library file holds every tag and rating the user has ever set, and none of that exists anywhere else. A crash or a power cut in the middle of a direct write leaves a truncated JSON file, which fails to parse on the next start, which falls back to an empty library. Writing to `library.tmp` and renaming it over the target makes the replacement atomic, so the old file survives until the new one is complete.
