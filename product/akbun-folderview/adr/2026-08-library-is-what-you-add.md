...
That decision is why `library.js` is loaded into the renderer as a plain script rather than reached through IPC. Sending the entries across the context bridge on every keystroke would reintroduce exactly the latency the design removes, and would do it for a filter that costs less than the message.
