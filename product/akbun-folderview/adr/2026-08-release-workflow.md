...
The pull request job runs the page tests with `node --test` and the model tests with `cargo test -p folderview-library`. Both suites are pure logic and neither needs a webview, which is why a pull request gets its answer on ubuntu rather than waiting on a Windows runner. The Windows runner exists to build the installer on the platform it targets instead of cross compiling it.
