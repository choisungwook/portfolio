...
const TEMP_PREFIX = 'akbun-ai-useage-update-';
...
test('pickDmg matches the running architecture', () => {
  const assets = [
    { name: 'akbun-ai-useage-0.2.0-arm64.dmg', browser_download_url: 'arm' },
    { name: 'akbun-ai-useage-0.2.0.dmg', browser_download_url: 'x64' },
    { name: 'akbun-ai-useage-0.2.0.zip', browser_download_url: 'zip' },
  ];
...
test('downloadDmg removes its temp dir when the download fails', async () => {
  ...
  await assert.rejects(() => downloadDmg('http://127.0.0.1:1/akbun-ai-useage.dmg'));
