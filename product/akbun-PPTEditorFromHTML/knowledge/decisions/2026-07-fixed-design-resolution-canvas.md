...
- 1차 구현은 학습지 전체를 iframe(srcdoc, sandbox)에 띄우고 그 위에서 편집했다. 사용자가 이 방향을 반려했고, 검증에서도 근본 결함이 드러났다: 템플릿 글씨는 뷰포트 기준(min(2vw,3.4vh))인데 페이지 폭 공식에는 고정 104px 항이 있어 창 크기마다 글씨:페이지 비율이 달라진다. 좌표를 얼려 놓으면 창 크기가 바뀔 때 줄바꿈이 변해 객체가 겹쳤다. 보정 상수로 쫓아가는 건 두더지 잡기였다.
