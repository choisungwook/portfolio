...
A read-only AWS viewer for macOS. The user picks a profile parsed from ~/.aws/config, signs in through IAM Identity Center (device authorization in a separate window), and browses EC2 instances with console-style detail tabs. Access keys are never read and no mutating API exists in the codebase.
