...
uv run --directory ../../ akbun-agent-analysiscode --config akbun-agent.toml status
```
...
- On master push, `.github/workflows/release-akbun-agent-analysiscode.yml` reads that
  version, fails early if the tag `akbun-agent-analysiscode-v<version>` already exists,
