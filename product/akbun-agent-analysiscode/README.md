...
topics as JSON, and the CLI links them into a service graph stored in a local
SQLite file (`knowledge/knowledge.db`) plus one markdown doc per service.
`ask` and `chat` then hand that graph to a fresh agent as context, so it
reasons about failure propagation across services and reads source files only
when it needs evidence. No vector database, no database server to run, no
embedding pipeline — see [adr/](./adr/) for why.
...
```bash
uv tool install "git+https://github.com/choisungwook/portfolio.git#subdirectory=product/akbun-agent-analysiscode/workspace"
```
...
```bash
akbun-agent-analysiscode learn
akbun-agent-analysiscode status
akbun-agent-analysiscode ask "Orders return 500, what is the likely cause?" --log error.log
akbun-agent-analysiscode chat
```
...
uv run --directory ../../ akbun-agent-analysiscode --config akbun-agent.toml learn
uv run --directory ../../ akbun-agent-analysiscode --config akbun-agent.toml ask \
