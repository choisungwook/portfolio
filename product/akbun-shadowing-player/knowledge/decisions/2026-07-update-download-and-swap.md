...
- dmg가 100MB를 넘으므로 임시 파일 정리를 세 겹으로 둔다. 교체 스크립트는 trap으로 어느 단계에서 실패하든 mount 지점과 작업 디렉터리를 지우고, 스크립트를 띄우기 전에 실패하면 main이 받아 둔 dmg를 지우며, 앱이나 스크립트가 강제 종료돼 남은 디렉터리는 다음 실행 때 앱 시작 시점에 훑어 지운다. 정리가 한 군데라도 빠지면 실패할 때마다 100MB가 쌓인다.
- 내려받기는 스트림으로 흘려 쓴다. arrayBuffer로 받으면 dmg 전체가 메모리에 올라간다.
