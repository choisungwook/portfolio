...
- The app's own control icon is subject to that same rule, so it can be invisible on a full bar. The shortcut and the window opening on launch are the recovery paths, not decoration.
- A global shortcut cannot be checked for conflicts. macOS lets a second registration of a combination another app owns succeed and never fire, so the combination is a user setting with an off switch rather than a constant.
...
| `Sources/akbun-mactaskbar/Hotkey.swift` | system-wide shortcut and its choices |
