...
GitHub pull request에서 terraform plan/apply를 실행하는 셀프호스팅 자동화 서버다. PR이 열리면 변경된 terraform 프로젝트를 자동으로 plan하고, 리뷰어가 comment로 `terraform apply`를 남기면 리뷰한 plan 파일을 그대로 apply한다.
...
4. `terraform apply` comment를 받으면 저장된 plan 파일을 apply한다. plan 이후 PR head가 바뀌었으면 거부하고 재plan을 요구한다. `terraform import`로 기존 리소스를 state에 넣을 수도 있다.
...
코드를 고치면 `Cargo.toml`의 version을 올린다. master에 merge되면 GitHub Actions가 그 버전으로 tag와 release(linux x86_64 바이너리)를 만들기 때문에, 버전이 그대로면 tag 충돌로 release가 실패한다.
