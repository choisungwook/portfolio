...
macOS menu bar app for area screenshots. Drag to select, and a floating preview appears in the bottom-left corner with four buttons laid out two by two: Save writes a png into the save directory, Copy puts the image on the clipboard, Edit opens the annotation editor, and Close keeps nothing. The menu bar icon also carries Check for Updates, which pulls the newest dmg from GitHub Releases and replaces the app in place.

## Editing

Edit opens the capture in an editor whose toolbar sits outside the image, with Save, Save as, Copy and Close pinned to the top right. The tools are select, rectangle, ellipse, line, one-way arrow, two-way arrow, text, an auto-incrementing numbered badge, pencil and crop, each in the chosen color. Each is an icon button with its name on hover, so the ten of them fit on one row however narrow the window is; the four action buttons keep their words, because a button that overwrites or discards should say which it does. Holding Shift squares a drag, so the ellipse becomes a circle and the line and arrows snap to the nearest 45 degrees. Cmd+Z undoes and Cmd+Shift+Z redoes, and everything is undoable: drawing, moving, resizing, restyling, deleting and cropping.
