...
description: GUI 프로세스의 PATH 밖에 설치된 git과 gh를 절대 경로로 찾아 실행한다.
tags: [desktop, cli, macos, github]
...
Finder에서 실행한 macOS GUI 앱은 터미널의 shell 초기화 파일을 읽지 않는다. 터미널에서는 gh가 동작해도 GUI 프로세스의 PATH에는 Homebrew 경로가 없어 설치되지 않은 것으로 오판할 수 있다.
