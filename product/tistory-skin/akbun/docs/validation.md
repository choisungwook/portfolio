...
- 프리젠테이션 모드에서 글 헤더와 본문만 남기는지. 스킨 헤더/푸터, 태그, 다른 버튼과 티스토리가 붙이는 좋아요/공유/CCL(`.container_postbtn`), 같은 카테고리 글(`.another_category`), 프로필 명함/구독 툴바(`[data-tistory-react-app]`, `.menu_toolbar`), 광고, 댓글을 모두 숨긴다
