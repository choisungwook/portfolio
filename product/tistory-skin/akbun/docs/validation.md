...
- 전체화면 지원 환경에서는 브라우저 전체화면으로 전환되고, API가 거부되면 집중 화면이 유지되는지
- 프리젠테이션 모드에서 글 헤더와 본문만 남기는지. 아래를 모두 숨긴다
  - 스킨: 헤더, 푸터, 태그, 목록으로 돌아가기와 Markdown 복사 버튼
  - 티스토리 주입: 좋아요/공유/CCL(`.container_postbtn`), 같은 카테고리 글(`.another_category`)
  - 티스토리 주입: 프로필 명함과 구독 툴바(`[data-tistory-react-app]`, `.menu_toolbar`)
  - 티스토리 주입: 광고(`.revenue_unit_wrap`, `.adsbygoogle`)와 댓글
- 프리젠테이션 모드에서 목차가 본문 왼쪽에 표시되는지
- 긴 목차 항목이 목차 영역 안에서 줄바꿈되고 본문을 침범하지 않는지
- 프리젠테이션 모드에서 본문이 목차 오른쪽부터 콘텐츠 영역 끝까지 채우는지
- 프리젠테이션 모드 종료 버튼으로 원래 화면으로 복귀하는지
- 브라우저 전체화면 진입에 성공한 경우 `Esc`로 원래 화면으로 복귀하는지
