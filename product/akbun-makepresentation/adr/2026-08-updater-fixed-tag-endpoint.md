...
The updater endpoint is `releases/download/akbun-makepresentation-updater/latest.json`. After tauri-action publishes a versioned release, the workflow copies its `latest.json` to that fixed tag with `gh release upload --clobber`, and the fixed release is created with `--latest=false`.
