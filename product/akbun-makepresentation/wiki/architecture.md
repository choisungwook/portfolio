...
Each document runs in its own OS process with two app sides, one JSON model between them, plus an optional external AI process. Settings and AI sessions belong to the document profile. Opening another file spawns the same executable with a file argument and leaves the current editor intact, with one exception: a window that shows no file adopts the file itself. `adopt_document` swaps the process's profile for the file's profile, the page loads the deck, reloads settings and restarts the App Server, and no second window appears. On macOS a double-clicked file arrives as an `Opened` run event rather than an argument; before the page has asked for its startup document that file simply becomes the startup document, afterwards the event is forwarded to the page, which applies the same rule ([decision](../knowledge/decisions/2026-09-document-process-and-profile-isolation.md)).
...
| Command | In | Out |
|---|---|---|
| `initial_document`, `adopt_document`, `launch_document` | optional path | startup path, this window's new document, or separate app process |
...
The chip row above the composer holds request modifiers, filtered to the current mode. Image mode never receives the measured reading, but its icon chip asks for the slide's words: `composeTurn` then lists the text of every non-code shape so the model can draw a symbol for what the slide says. A chip is a toggle: clicking one leaves the textarea untouched and only reaches the model at send time, when `composeTurn` assembles the mode rules, the active chips' instructions, the slide reading, the diagram style and the user's text into one turn. Chips stay on across sends, because "draw it, then tidy it, then tidy it again" is one intent held over three turns.
